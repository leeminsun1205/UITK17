#include <bits/stdc++.h>
using namespace std;
class Fraction // cấu trúc phân số có dạng tử/mẫu;
{
    private:
...
Fraction Fraction::Shorten(Fraction &F) // hàm rút gọn phân số
{
...
Fraction operator + (Fraction F1, Fraction F2) // hàm nạp chồng toán tử cộng 2 phân số
{
    ...
    return Shorten(Sum); // trả về phân số tổng ở dạng rút gọn
}
Fraction operator - (Fraction F1, Fraction F2) // hàm nạp chồng toán tử trừ 2 phân số
{
    ...
    return Shorten(Difference); // trả về phân số hiệu ở dạng rút gọn
}
Fraction operator * (Fraction F1, Fraction F2) // hàm nạp chồng toán tử nhân 2 phân số
{
    ...
    return Shorten(Product); // trả về phân số tích ở dạng rút gọn
}
Fraction Fraction::operator / (Fraction another) // hàm nạp chồng toán tử chia 2 phân số
{
    Fraction Quotient;
    Quotient.numerator=this->numerator*another.pattern;
    Quotient.pattern=this->pattern*another.numerator;
    if (Quotient.pattern==0) // nếu mẫu số của phân số thương là 0 thì xuấtt ra không thể chia 2 phân số
        cout << "Khong the chia 2 phan so!";
    else 
        cout << "Thuong 2 phan so la: " << Quotient;
    return Shorten(Quotient); // trả về phân số thương ở dạng rút gọn
}
int main()
{
    Fraction F1,F2;
    ...
    cout << "Phan so thu nhat la: " << Shorten(F1) << endl;
    cout << "Phan so thu hai la: " << Shorten(F2) << endl;
